...
TaskSystemParallelSpawn::TaskSystemParallelSpawn(int num_threads): ITaskSystem(num_threads) {
    ...
    // (requiring changes to tasksys.h).
    //
	this->num_T = std::max(1, num_threads);
        this->threads = new std::thread[this->num_T - 1];
...
//	}
//}
void TaskSystemParallelSpawn::workFunc(IRunnable* runnable, int total) {
	while(true) {
		mutex_->lock();
		int current = counter;
		if (counter < total) {
		...
		runnable->runTask(current, total);
	}
}	
...
void TaskSystemParallelSpawn::run(IRunnable* runnable, int num_total_tasks) {
	...
	// Main works
	workFunc(runnable, num_total_tasks);	    
